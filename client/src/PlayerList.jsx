...
const PlayerList = ({ 
  ...
  console.log("PlayerList render with:", {
  ...
  const voiceChatPlayers = players.filter(player => {
    const isInVoiceChat = playersInVoiceChat[player.id] === true;
    console.log(`Player ${player.name} (${player.id}) in voice chat: ${isInVoiceChat}`);
  ...
  const handleRejoinVoiceChat = () => {
    if (socket && roomId) {
      console.log(`Sending join-voice-chat event to the server for ${currentPlayerId} in room ${roomId}`);
      socket.emit('join-voice-chat', { roomId });
      
      // Update local state immediately
      if (window.updateVoiceChatPlayers) {
        console.log(`Updating local voice chat state to include self: ${currentPlayerId}`);
        window.updateVoiceChatPlayers({
          [currentPlayerId]: true
        });
      }
      
      // Show the voice chat modal if needed
      if (window.showVoiceChatModal) {
        window.showVoiceChatModal();
      }
      
      // Call direct join if available
      if (window.joinVoiceChat) {
        window.joinVoiceChat();
      }
      
      // Force re-join after a short delay to ensure server has processed our join
      setTimeout(() => {
        console.log(`Re-sending join-voice-chat event to ensure server registration`);
        socket.emit('join-voice-chat', { roomId });
      }, 500);
    }
  };
  ...
  const handleLeaveVoiceChat = () => {
    if (socket && roomId) {
      console.log("Leaving voice chat from player list");
      ...
      // Call global cleanup function if available
      if (window.cleanupVoiceChat) {
