...
io.on('connection', (socket) => {
  ...
  socket.on('join-voice-chat', (data) => {
    ...
    // Add player to voice chat users
    if (!voiceChatUsers.has(roomId)) {
      voiceChatUsers.set(roomId, new Set());
    }
    
    const roomVoiceUsers = voiceChatUsers.get(roomId);
    roomVoiceUsers.add(socket.id);
    
    console.log(`Player ${socket.id} (${player.name}) joined voice chat in room ${roomId}`);
    
    // Notify all clients in the room about the updated voice chat users
    ...
    // Notify other clients in voice chat about the new user
  ...
  socket.on('leave-voice-chat', (data) => {
    ...
    if (voiceChatUsers.has(roomId)) {
      ...
      console.log(`Player ${socket.id} (${player.name}) left voice chat in room ${roomId}`);
