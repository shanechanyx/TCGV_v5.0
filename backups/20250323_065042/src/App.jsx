...
function App() {
  ...
  const [registeredUsers, setRegisteredUsers] = useState([
    ...
    { userId: 'user1', password: 'pass1', email: 'user1@example.com' },
    { userId: 'user2', password: 'pass2', email: 'user2@example.com' },
    { userId: 'takochn', password: 'takochn', email: 'takochn@example.com' }
  ]);
  ...
  const validateLogin = () => {
    ...
    if (loginMode === 'login') {
      ...
      // Guest login needs at least a user ID
      if (!userId) {
        setError('Please enter a Guest User ID');
        return false;
      }
      setSuccessMessage('Logging in as Guest');
      return true;
    }
  ...
  }, [showProximityPrompt, nearbyPlayers]);

  // Handle interaction menu options
  const handleInteraction = (action) => {
    if (!selectedPlayer) return;
    
    switch(action) {
      case 'addFriend':
        console.log(`Friend request sent to ${selectedPlayer.name}`);
        // Here you would implement the friend request sending logic
        // For example:
        // socket.emit('friendRequest', { targetId: selectedPlayer.id });
        break;
      case 'viewProfile':
        console.log(`Viewing profile of ${selectedPlayer.name}`);
        // Here you would implement the profile viewing logic
        break;
      case 'openWallet':
        console.log(`Opening wallet of ${selectedPlayer.name}`);
        // Here you would implement the wallet opening logic
        // For example opening in a new window:
        window.open(`/wallet?playerId=${selectedPlayer.id}`, '_blank');
        break;
      default:
        break;
    }
                  ...
                  {loginMode === 'guest' && (
                    <div className="guest-info">
                      <div className="hero-message">
                        <p>You're entering as a guest. Your session will not be saved after you leave.</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <Step>
                ...
                <div className="hero-form-group">
                  <div className="hero-summary">
                    ...
                    <p><strong>Role:</strong> {role ? (role === 'roleA' ? 'Role A' : 'Role B') : 'Not set'}</p>
  ...
  return (
    <div className={`hero-room-container ${isSidebarCollapsed ? 'sidebar-collapsed' : ''} ${isChatCollapsed ? 'chat-collapsed' : ''} ${isSidebarCollapsed && isChatCollapsed ? 'both-collapsed' : ''}`}>
      ...
      {/* Interaction menu */}
      {showInteractionMenu && selectedPlayer && (
        <div className="interaction-menu">
          <div className="interaction-menu-header">
            Interact with {selectedPlayer.name}
          </div>
          <div className="interaction-menu-options">
            <button 
              className="interaction-option"
              onClick={() => handleInteraction('addFriend')}
            >
              Add Friend
            </button>
            <button 
              className="interaction-option"
              onClick={() => handleInteraction('viewProfile')}
            >
              Profile
            </button>
            <button 
              className="interaction-option"
              onClick={() => handleInteraction('openWallet')}
            >
              Wallet
            </button>
          </div>
